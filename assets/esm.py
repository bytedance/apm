...
class FAEsmModel(EsmModel):
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = EsmEmbeddings(config) # token_dropout is True, position encoding is rotary encoding
        # print('*'*10, self.embeddings.position_embedding_type)
    ...
    def forward(
        ...
        print('#'*10, extended_attention_mask.shape)
        ...
        ), (embedding_output, cu_seqlens, max_seqlen, _, output_pad_fn)
...
class FAEsmForMaskedLM(EsmForMaskedLM):
    ...
    def forward(
        ...
        outputs, intermediate_ = self.esm(
            input_ids,
            attention_mask=attention_mask,
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=encoder_attention_mask,
    ...
    def customized_forward(
        ...
        attention_mask = input_ids.ne(self.pad_id)
        outputs = self.esm.customized_forward(
            input_ids,
            attention_mask=attention_mask,
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=encoder_attention_mask,
